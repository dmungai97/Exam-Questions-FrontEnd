<template>
  <b-modal v-model="modalShow" @cancel="handleCancel">
    <template v-slot:modal-header>
      <h5>{{ $t('schoolEntry.title') }}</h5>
    </template>
    <template v-slot:default>
      <TextInput v-model="value.name" :label="$t('schoolEntry.name.label')" :placeholder="$t('schoolEntry.name.placeholder')" type="text"></TextInput>
    </template>
    <template v-slot:modal-footer="{ cancel }">
      <b-button size="sm" variant="secondary-outline" @click="ok()">
        {{ $t('schoolEntry.buttons.save') }}
      </b-button>
      <b-button size="sm" variant="secondary-outline" @click="cancel()">
        {{ $t('schoolEntry.buttons.cancel') }}
      </b-button>
    </template>
  </b-modal>
</template>
